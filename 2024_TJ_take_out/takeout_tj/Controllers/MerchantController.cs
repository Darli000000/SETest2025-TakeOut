...
namespace takeout_tj.Controllers
{
    ...
    public class MerchantController : Controller
    {
        ...
        public MerchantController(ApplicationDbContext context, MerchantService merchantService, string uploadsFolder = null)
        {
            _context = context;
            ...
            if (!Directory.Exists(_uploadsFolder))
            {
                Directory.CreateDirectory(_uploadsFolder); // 如果想 mock，这里也要抽象出去
            }
        }
        ...
        public IActionResult EditMerchant([FromBody] MerchantDBDto dto)
        {
            ...
            try
            {
                tran = _context.Database.BeginTransaction(); // 开始事务 
            }
            catch
            {
                // 在测试中不启用事务
        ...
        public IActionResult deletePaidOrder(int orderId)
        {
            ...
            try
            {
                ...
                if (orderCoupon != null)
                {
                    //Console.WriteLine("Debug info1; " + orderId);
                    var userCoupon = _context.UserCoupons.FirstOrDefault(m => m.UserId == orderCoupon.UserId && m.CouponId == orderCoupon.CouponId);
                    //Console.WriteLine("Debug info2; " + orderCoupon.UserId + userCoupon.UserId);
                    if (userCoupon != null)
                    {
        ...
        public async Task<IActionResult> GetMerOrdersWithinThisMonth(int merchantId)
        {
            try
            {
                ...
                var orders = await _context.Orders
                    ...
                    .Select(o => o.OrderId)
                    .ToListAsync();
                /* Attention： 这里测试找到了错误，应该使用!orders.Any(), 而非!orderMerchant.Any() */
                if (!orders.Any())
                {
